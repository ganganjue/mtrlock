...
/**
 * 客户端团队名前缀（头顶名字 / 客户端显示名用）。
 *
 * <p>服务端前缀（{@link TeamPrefix}）走 {@code TeamData}，取“最早加入的团队”；
 * 客户端没有 {@code createdAt} 信息，只有 S2C 同步来的
 * {@code teamId → members} 与 {@code teamId → name}，因此这里用
 * <b>teamId 字典序最小</b>的所属团队近似“最早加入”。视觉上可用。</p>
 *
 * <p>数据来源：{@link ClientOwnership#getTeamMembers()} /
 * {@link ClientOwnership#getTeamName(String)}，由 {@code MtrlockClient} 在收到
 * {@code mtrlock:sync_ownership} 时填充。查不到 → {@link #NO_TEAM}，永不返回 null。</p>
 *
 * <p>纯静态缓存读取，不触碰 {@code FabricLoader} / {@code TeamData}，可纯 JVM 测试。</p>
 */
public final class ClientTeamPrefix {

    /** 没有任何团队时使用的前缀（与服务端 {@link TeamPrefix#NO_TEAM} 一致）。 */
